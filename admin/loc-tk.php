...
<head>
    ...
    <style>
       ...
       /* Bỏ khoảng trắng quanh navbar */
        ...
        .table-wrapper {
            margin: 0 25px; /* Cách trái phải */
            overflow-x: auto; /* Phòng trường hợp bảng dài bị tràn */
            }
        /* NÚT CHI TIẾT*/
        .btn-edit {
            padding: 6px 12px;
...
<body>
    ...
    <form class="search-form d-flex flex-grow-1" action="index.php?act=search-tk" method="post" onsubmit="return validateSearch()">
        <input class="form-control me-2 short-search" type="search" name="nhaptim" placeholder="Tìm kiếm sản phẩm..." aria-label="Search">
    ...
    <form method="GET" action="index.php" class="filter-form" style="margin-bottom: 15px;">
        ...
        <select name="role" onchange="this.form.submit()" class="form-select" style="min-width: 150px;">
           ...
           <?php       
                if (isset($tk) && is_array($tk)) {
                    ...
                    $i = 1;
                    foreach ($tk as $k) {
                        // Đổi số vai trò sang tên dễ đọc
                        if ($k['TK_VAITRO'] == 0) {
                            $vaitro = "Nhân viên";
                        } elseif ($k['TK_VAITRO'] == 1) {
                            $vaitro = "Khách hàng";
                        } elseif ($k['TK_VAITRO'] == 2) {
                            $vaitro = "Quản trị";
                        } else {
                        ...
                        echo '<tr>
                               ...
                               <td>
                                    ...
                                    <input type="hidden" name="nv_id" value="' . $k['TK_ID'] . '">
                                    <select name="trangthai" class="form-select form-select-sm" onChange="capNhatStatusTk('.$k['TK_ID'].', this.value)">
                                        <option value="Còn hoạt động" ' . ($k['TK_TRANGTHAI'] === 'Còn hoạt động' ? 'selected' : '') . '>Còn hoạt động</option>
                                        <option value="Ngưng hoạt động" ' . ($k['TK_TRANGTHAI'] === 'Ngưng hoạt động' ? 'selected' : '') . '>Ngưng hoạt động</option>
                                    </select>
                                              
                                            </td>
                               <td> <a class="btn-edit" href="index.php?act=detail-tk&id=' . $k['IDLIENKET'] . '&vaitro=' . $k['TK_VAITRO'] . '">Chi tiết</a></td>
                            </tr>';
                        $i++;
                    }
                    echo '</table>';
                } else {
                    echo "<p>Không có dữ liệu tài khoản.</p>";
                }
                ?>
        ...
        function capNhatStatusTk(idtk, trangthai) {
            fetch('index.php?act=capnhat_trangthai_tk', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded'
                },
                body: `idtk=${idtk}&trangthai=${trangthai}`
            })
